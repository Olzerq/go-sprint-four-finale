...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	return (runningCaloriesMeanSpeedMultiplier * runningSpeed * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	walkingSpeedInKm := meanSpeed(action, duration)
	walkingSpeedInM := walkingSpeedInKm * kmhInMsec
	return (walkingCaloriesWeightMultiplier*weight + (math.Pow(walkingSpeedInM, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
